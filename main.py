...
def load_sinkignore():
    ...
    if SINKIGNORE_PATH.exists():
        with open(SINKIGNORE_PATH, "r") as fp:
            for line in fp:
                line = line.strip()
...
def save_trusted_devices(devices):
    with open(DEVICES_FILE, "w") as fp:
        json.dump({"trusted_devices": list(devices.values())}, fp, indent=2)
...
def sync_folder_to_peer(peer):
    peer_id = peer["ip"]
    print(f"[sink] Performing initial sync with {peer['name']} ({peer_id})")
...
def sync_to_peers(rel_path, abs_path, filehash):
    ...
    for peer in peers:
        ...
        try:
            ...
            print(f"ERROR: sync_to_peers failed due to {err}")
...
def delete_on_peers(rel_path):
    ...
    for peer in peers:
        ...
        try:
            ...
            print(f"ERROR: delete_on_peers failed due to {err}")
...
def generate_self_signed_cert():
    ...
    if CERT_FILE.exists() and KEY_FILE.exists():
        ...
        print("[sink] SSL certificates already exists.")
        return
            ...
            with peer_lock:
                if action == "request_trust":
                    ...
                    if is_device_trusted(device_id):
                        ...
                        if state == "requested":
                            print(f"[sink] Manual trust with {remote_device_name} ({device_id}).")
...
def run_http_server():
    ...
    print(f"[sink] Listing on the port {PEER_PORT}")
    ...
    def add_service(self, zc, type_, name):
        ...
        if info:
            ...
            try:
                ...
                print(f"ERROR: PeerListener.add_service failed due to {err}")
...
def start_discovery():
    ...
    print(f"[sink] Zeroconf discovery started")
                ...
                with peer_lock:
                    if is_device_trusted(peer_id):
                        ...
                        if state == 'pending':
                            ...
                            add_trusted_devices(peer_id, peer_info['name'], peer_info['ip'])
